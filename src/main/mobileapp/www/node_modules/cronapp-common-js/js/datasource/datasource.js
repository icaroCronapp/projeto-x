...
.factory('DatasetManager', ['$http', '$q', '$timeout', '$rootScope', '$window', 'Notification', 'SyncService',  function($http, $q, $timeout, $rootScope, $window, Notification, SyncService) {
  ...
  var DataSet = function(name, scope, fetchOnVisible) {
    ...
    this.sendODataFiles = function(odataFile, obj, callback, afterUpdate) {
      if (obj && odataFile && odataFile.length > 0) {
        ...
        reduce(odataFile, function (of, resolve) {
          ...
          let urlSend = (window.hostApp || "") + url + '/' +  of.field + '/$value';
          
          if (!_self.isOfflineMode()) {
            var file = of.value;
            var xhr = new XMLHttpRequest;
            xhr.open('PUT', urlSend);
            ...
            SyncService.toQueue(config);
            cronapi.internal.fileToBase64(of.value, (base64) =>{
              obj[of.field] = base64;
              if (callback) {
                callback({ field: of.field, data: obj });
              } 
              resolve();
            });
          }
    ...
    this.retrieveDefaultValues = function(obj, callback) {
      if (this.isEventData() || this.isLocalData() || this.isOfflineMode()) {
        this.$scope.safeApply(function() {

          this.active = {};
          if (this.isLocalData() || this.isOfflineMode()) {
    ...
    this.startInserting = function(item, callback) {
      ...
      this.operationPouchDB = function (verb, value, callback) {
        if (SyncService.synchronizing) 
          return;

        if (this.hasOfflineSupport() || this.isLocalData()) {
          if (verb && verb !== 'GET') {
            let db = this.getPouchDB(); // db origin
            if (verb == 'DELETE') {
              db.get(value._objectKey).then(function (doc) {
                return db.remove(doc);
              }.bind(this)).then(callback).catch(function (err) {
                console.error(err);
              });
            } else {
              this.$scope.cronapi.internal.updatePouchDB(db, [value], callback);
            }
          }
        }
      };      

    this.getPouchDB = function () {
      return cronapi.internal.getPouchDB(_self.localDBName);
    };

    this.isNavigatorOffline = function () {
      return !navigator.onLine;
    };

    this.hasOfflineSupport = function () {
      return this.offline;
    };

    this.isOfflineMode = function () {
      return this.isNavigatorOffline() && this.hasOfflineSupport();
    };
    ...
    this.remove = function(object, callback, forceDelete, onSuccess, onError, silent) {
      ...
      if (!forceDelete && !silent && this.deleteMessage && this.deleteMessage.length > 0) {
        ...
        window.cronapi.notification.confirmDialogAlert("warning", this.translate.instant("Users.view.Confirmacao"), this.deleteMessage, modalButtons);
      } else {
            ...
            if (this.isEmpty(value) && (strategy == 'ignore' || strategy == 'clean' || strategy == 'wait')) {
              ...
              if (!this.isEmpty(value)) {
                ...
                if (canContinue) {
                  ...
                  result += `${arg.left} ${getQueryOperator(arg.type)} null`;
                }
              }
            }
          }
    ...
    this.fetch = function(properties, callbacksObj, isNextOrPrev, fetchOptions, silent, fromVisibleEvent) {
      ...
      var sucessHandler = function(data, headers, raw) {
          ...
          if (data) {
            if (Object.prototype.toString.call(data) !== '[object Array]') {
              if (data && data.links && Object.prototype.toString.call(data.content) === '[object Array]') {
                ...
                data = this.$scope.cronapi.internal.normalizePouchDBData(data.rows, true);
              }
        ...
        if (!this.isOfflineMode() && this.hasOfflineSupport()) {
          var db = this.getPouchDB(); 
          this.$scope.cronapi.internal.updatePouchDB(db, data);
        }
  ...
  this.storeDataset = function(dataset) {
    this.datasets[dataset.name] = dataset;
      ...
      this.initDataset = function(props, scope, $compile, $parse, $interpolate, instanceId, translate) {
        ...
        let canStoreDataset = true;
        //Validate if already exist datasource with a same name from the modal
        if ($rootScope[props.name] && !$rootScope[props.name].insideModal && props.insideModal) {
          canStoreDataset = false;
          console.log(`%c${translate.instant("Datasource.Error").toUpperCase()} - ${translate.instant("Datasource.Duplicate").toUpperCase()}${props.name.toUpperCase()}`, "background: #000; color: #FFF; font-size:14px;");
        }
        else {
        ...
        dts.offline = props.offline;
        dts.insideModal = props.insideModal;
        ...
        dts.dependentLazyPostField = props.dependentLazyPostField;
        dts.headers = props.headers;
    ...
    link: function(scope, element, attrs) {
      ...
      var renameTag = function (targetSelector, scope, newTagString) {
        $(targetSelector).each(function(){
          ...
          $newElem.attr('inside-modal', $(element).closest('.modal').length > 0);
...
app.directive('crnRepeat', function(DatasetManager, $compile, $parse, $injector, $rootScope) {
  return {
    ...
    link: function(scope, element, attrs, controllers, transclude) {
      ...
      const indexAttribute = (currEle, dataRepeaterId, attribute) => {
        currEle.parent().find(`[data-repeater=${dataRepeaterId}]`).each((idx, child) => {
          let $child = $(child);
          $child.find(`[${attribute}]`).each((index, subChild) => {
            let $subChild = $(subChild);
            let currId = $subChild.attr(`${attribute}`);
            let newId = `${currId}-cronapp-${idx}`;
            $subChild.attr(`${attribute}`, newId);
          });
        });
      };

      datasourceRepeat++;
      let dataRepeaterId = `datasourceRepeat${datasourceRepeat}`;
      if (attrs.crnRepeat) {
        scope.data = DatasetManager.datasets;
        if (scope.data[attrs.crnRepeat]) {
          scope[dataRepeaterId] = scope.data[attrs.crnRepeat];
        } else {
          scope[dataRepeaterId] = {};
          var value = $parse(attrs.crnRepeat)(scope);
          if (value instanceof String && value.startsWith("[") && value.endsWith("]")) {
            value = eval(value);
          }
          scope[dataRepeaterId].data = value;
        }
        element.attr('ng-init', attrs.crnRepeat + ' = {}; ' + attrs.crnRepeat + '.active = rowData');
        element.attr('ng-repeat', `rowData in ${dataRepeaterId}.data`);
        element.attr('data-repeater', dataRepeaterId);
      }

      var tagName = element[0].tagName;
      $compile(element, null, 9999998)(scope);
      scope.$watchCollection(`${dataRepeaterId}.data`, function (newVal, oldVal) {
        if (tagName.toLowerCase() == "ion-slide") {
          var $ionicSlideBoxDelegate = $injector.get('$ionicSlideBoxDelegate');
          $ionicSlideBoxDelegate.update();
        }

        indexAttribute(element, dataRepeaterId, 'id');
        indexAttribute(element, dataRepeaterId, 'for');
      });
    }
  };
})
 ...
 */
.directive('cronappDatasource', ['DatasetManager', '$timeout', '$parse', 'Notification', '$translate', '$location','$rootScope', '$compile', '$interpolate', 'SyncService',
  function(DatasetManager, $timeout, $parse, Notification, $translate, $location, $rootScope, $compile, $interpolate, SyncService) {
  return {
    restrict: 'E',
    scope: true,
    template: '',
...
angular.module('sync.service', []).service('SyncService', function($http) {
  
  let syncService = {
    start: function () {
      this.dbSync = this.getSyncServicePouchDB();
      this.observeConnection();
    },
    observeConnection: async function() {
      let observing = setInterval(() => this.isNavigatorOffline() || this.synchronizeOfflineData(), 1000 * 15);
    },
    toQueue: function(config) {
      let doc = { data: config, created: Date.now() };
      this.dbSync.post(doc).catch((err) => console.error(err));
    },      
    synchronizeOnError: async function(e, doc, docs) {
      this.synchronizing = false;
      console.log('Error synchronizing doc', doc, 'response', e);
    },        
    synchronizeOnSuccess: async function(doc, docs) {
      try {
        await this.dbSync.remove(doc);
        this.removeArray(doc, docs);
        if (docs.length > 0) {
          this.sendData(docs[0], docs); 
        }
        else {
          this.synchronizing = false;  
        }
    ...
    canByPassError: function(e) {
      let result = false;
      if (e.data && e.data.error && e.data.error.message && e.data.error.message.value) {
        let value = e.data.error.message.value;
        switch (value) {
          case "Não foi possível encontrar a entidade requisitada.":
          case "Requested entity could not be found.":
            result = true;
          break;
        }
      }
    ...
    sendData: async function(doc, docs) {
      let urlOrign;
      let docData = doc.data;
      $http(docData)
        .then(() => this.synchronizeOnSuccess(doc, docs))
        .catch((e) => (this.canByPassError(e) && this.synchronizeOnSuccess(doc, docs))  || this.synchronizeOnError(e, doc, docs));
      console.log(`Synchronization - sended record number ${docs.length}`);
    },
    optimizeQueue: function(result) {
      let breakDocs = result.rows.filter((cur)=> cur.doc.data.method === 'DELETE' );
      breakDocs.forEach(async (r) => {
        let toDelete = result.rows.filter((cur) => {
          let row = cur.doc.data.originalObject || cur.doc.data.rawData;
          let rowBreak = r.doc.data.originalObject || r.doc.data.rawData;
          return row && rowBreak && row._objectKey === rowBreak._objectKey && cur.doc.data.method !== 'DELETE'
        });
        toDelete.forEach(async (del) => {
          this.dbSync.remove(del.doc)
          this.removeArray(del, result.rows);
        })
      });          

      let docs = [];
      result.rows.forEach((r) => docs.push(r.doc));          
      docs.sort((a, b) => a.created - b.created);
      return docs;
    },
    synchronizeOfflineData: async function() {          
      try {    
        if (!this.synchronizing) {
          let result = await this.dbSync.allDocs({ include_docs: true, attachments: true });
          if (result.rows.length) {
            console.log('Synchronization started');
            this.synchronizing = true;
            let docs = this.optimizeQueue(result);
            this.sendData(docs[0], docs);
          }
        }
      }
    ...
    removeArray(doc, docs) {
      if (doc && docs) {
        let idx = docs.indexOf(doc);
        if (idx > -1) docs.splice(idx, 1);
      }
    },  
    isNavigatorOffline: function() {
      return !navigator.onLine;
    },
    getSyncServicePouchDB: function () {
      return cronapi.internal.getPouchDB('syncService');
    },
    synchronizing: null,
    dbSync: null,
    lastStatus: null,
